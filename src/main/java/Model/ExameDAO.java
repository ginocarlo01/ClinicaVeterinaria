...
// CRUD    
    ...
    public Exame retrieveByIdConsulta(int id_consulta) {
        List<Exame> exames = this.retrieve("SELECT * FROM exame WHERE id_consulta = " + id_consulta);
        return (exames.isEmpty()?null:exames.get(exames.size()-1));
    }
    
    public List retrieveAllByIdConsulta(int id_consulta) {
        List<Exame> exames = this.retrieve("SELECT * FROM exame WHERE id_consulta = " + id_consulta);
        return exames;
    }
    ...
    public List retrieveBySimilarName(String nome) {
     ...
     public Exame retrieveByName(String nome) {
        List<Exame> exames = this.retrieve("SELECT TOP 1 * FROM exame WHERE nome LIKE '%" + nome + "%'");
        return (exames.isEmpty()?null:exames.get(0));
    }    
